...
const calculateScore = (grid, dictionary) => {
  ...
  const validWords = words.filter(
    (word) => dictionary.has(word) && word.length >= MIN_WORD_LENGTH
  );
...
const findLeastUsedLetterOnGrid = (grid) => {
  ...
  const leastUsed = Array.from(gridLetters).reduce(
    (least, letter) => {
      const frequency =
        letterFrequencies.find((entry) => entry.letter === letter)?.weight ||
        Infinity;
      return frequency < least.frequency ? { letter, frequency } : least;
    },
